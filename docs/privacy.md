...
| Dashboard **Healthchecks** ping (#79) | `hc-ping.com` (or self-hosted) | nothing about you — the endpoint sees a **Tor exit**, not your IP | ✅ Tor (`socks5h`) by default | opt-in (`healthchecks.enabled`, off by default) | `healthchecks.tor: false` pings over clearnet (exposes your IP — needed for a LAN self-hosted instance) |
...
- [ ] Leave Telegram (#121) off unless you accept its clearnet IP exposure. Healthchecks (#79) is Tor-routed by default, so it's safe to enable — just don't set `healthchecks.tor: false` (or point it at a LAN self-hosted instance) unless you accept exposing your IP to the ping endpoint.
