...
| Dashboard **XvB raffle registration** (#263) | `xmrvsbeast.com` | your Monero **wallet** (no longer your IP) | ✅ Tor (`socks5h`, same path as the stats fetch) | on, only if XvB enabled **and** the endpoint is configured (`XVB_SUBMIT_URL`); fires once you have a PPLNS share | `XVB_ENABLED=false`, or leave `XVB_SUBMIT_URL` unset, to stop it |
