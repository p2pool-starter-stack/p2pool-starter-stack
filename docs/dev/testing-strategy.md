...
| Appliance certificate (#1132/#1141): one shared name-list builder feeds both Caddy's site list and the certificate's SAN list, the mint re-mints only on a real list change (not a `hostname -I` re-order) and reuses otherwise, `doctor` FAILs on a name the certificate doesn't cover or a certificate within 30 days of expiry, WARNs (never FAILs) on an unreadable certificate file; the name list excludes BOTH compose bridges' gateways (mining_net's config-known prefix and proxy_net's engine-queried, auto-assigned one, #345) so a cert minted before `up` creates them still agrees with doctor's live re-derivation after `up` — the reboot-leg regression where a healthy still-syncing box FAILed the commit gate | sourced fns / fixture certs (real `openssl`) / stubbed `docker`+`podman` `network inspect` | 1 ✅ (`tests/stack`) · 4 (deferred — a real Caddy TLS handshake surviving a moved DHCP lease needs the KVM battery / bench) |
...
| Hugepages sizing on the FIRST boot: pool intact on the 16 GiB guest. This does not distinguish the sizing unit running and correctly changing nothing from the unit never running at all — the baked sysctl reserves the same pool either way, so `--phase boot`'s check alone can't tell the two apart (tracked as #1212) | battery `--phase boot` | 4 ✅ |
