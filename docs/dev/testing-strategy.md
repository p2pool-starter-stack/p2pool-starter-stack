...
| Dashboard exposure boundary (#1021/#1049): the rendered Caddyfile publishes the LAN v4 and the ULA but never the globally-routable v6; the bind line excludes it; a pinned `dashboard.host` does not reopen it; a box with only a public address still binds loopback; EVERY site block carries a bind (the count invariant that caught the unbound HTTPS onion vhost); the onion vhosts bind the container-bridge gateway and not a wildcard. **Known hole:** the LAN-bind fixture pins an IP literal, where `hostname -I` and `$site_hosts` render identically, so the #1089 mutation still stays green — pinning a NAME is what closes it | `tests/stack/run.sh` (sourced `generate_caddyfile`, stubbed `hostname`) | 1 ✅ |
