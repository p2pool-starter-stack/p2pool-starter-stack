...
| Restore leg: a real encrypted backup off a live machine, uploaded instead of the form on a fresh install, wallet + Tor identity restored not regenerated. The final verdict (#1091) requires the stack to actually come back up and a value sourced from the restored config — p2pool's own live stratum stats, `/api/state`'s `.stratum.wallet` — to match, not just `config.json` landing on disk: that alone proves the archive was unpacked, not that anything is RUNNING it | battery `--phase install` | 4 (added — unverified until the next battery run) |
