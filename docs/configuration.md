...
| `dashboard.check_for_updates` | `false` _(off)_ | When `true`, the dashboard periodically asks GitHub whether a **newer Pithead release** exists and, if so, shows a header badge linking to it (e.g. "New release v1.4.0 available"). **Notify-only** — it never updates anything; you upgrade with `./pithead upgrade` on your own terms. **Off by default** so the appliance never contacts GitHub unbidden. When on, the check is **routed over Tor** (the same bridge SOCKS as the XvB fetch) so it can't reveal your IP to GitHub, is cached (hourly), and fails silently offline. See [Privacy › Runtime egress](privacy.md#runtime-egress). |
