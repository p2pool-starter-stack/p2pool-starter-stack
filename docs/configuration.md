...
| `dashboard.control.enabled` | `false` _(off)_ | Default off. `true` turns on the in-dashboard **config editor** — edit `config.json` from the browser and apply it, instead of editing the file and running `./pithead apply` by hand. It writes changes through an audited host-side runner that re-validates every change and runs `pithead apply` for you; the dashboard itself never touches the host. pithead **fails closed**: enabling this without a `dashboard.auth.password` aborts. It also needs a systemd host (the runner is a systemd unit). See [The config editor](dashboard.md#the-config-editor) and [Security › The host-mutation channel](../SECURITY.md#the-host-mutation-channel-33). |
