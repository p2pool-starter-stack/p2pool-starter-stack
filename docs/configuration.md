...
| `tari.mem_limit` | `auto` | A **safety ceiling** on the Tari container's memory. Tari's memory grows over time (it's commonly seen using 10+ GB), and on a small host an unbounded Tari can OOM the whole stack. This cap lets Tari restart *itself* cleanly under pressure — Monero/P2Pool keep mining throughout — instead of taking the machine down. `auto` is deliberately generous: it reserves ~25% of host RAM (at least 2 GB) for Monero/P2Pool/Tor/the OS and gives Tari the rest (~75%) — roughly **12 GB on a 16 GB host, 24 GB on 32 GB**. So Tari runs freely and only restarts on a genuine runaway. To bound it tighter (e.g. to free RAM for other apps on the box), set any Docker memory value such as `"8g"` or `"4096m"`; raise it again if Tari restarts too often. (Tari officially needs ≥4 GB, 8 GB+ recommended. Set it too low and Tari may OOM-loop during initial sync. Getting a clean restart relies on disabling swap, which needs `swapaccount=1` on older cgroup-v1 hosts; cgroup-v2 honors it by default.) |
