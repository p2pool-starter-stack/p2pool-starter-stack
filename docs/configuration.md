...
| `monero.mem_limit` | `auto` | Upper limit on the monerod container's memory, so a leak/runaway OOM-restarts monerod alone instead of the host's OOM-killer picking a victim. `auto` is a generous ceiling (4 GB) that won't trip during normal sync — monerod's blockchain DB is reclaimable page cache, not counted RSS. Lower it only to free RAM; **raise it** if a low-RAM host OOMs monerod during the heavy initial sync. Accepts any Docker memory value, e.g. `"6g"`. (The dashboard, P2Pool, Tor, and the proxies carry their own conservative ceilings in `docker-compose.yml`, overridable via `*_MEM_LIMIT` env vars.) |
