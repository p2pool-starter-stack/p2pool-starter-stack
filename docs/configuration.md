...
The table below has ~113 keys across 13 sections — every leaf key of
[`config.reference.json`](../config.reference.json), not counting its deprecated `xmrig_proxy.*`
...
| `telegram.control.allowed_ids` | `[]` | The numeric Telegram **user ids** allowed to issue control commands. Being in the configured `chat_id` is not enough; every other sender is dropped silently. Empty (the default) keeps the whole feature off. |
