...
| `dashboard.control.enabled` | `false` _(off)_ | Security-relevant, default off. `true` turns on the dashboard's **configuration editor** (#33): a Configuration page that edits `config.json` and applies it through a host-side runner. The dashboard container only stages typed change requests into a spool directory (`./data/control`); a root systemd path unit (`pithead-control.path`) runs `pithead apply` after re-validating the request — the container itself can never run commands on the host. pithead **fails closed**: enabling this without a `dashboard.auth.password` is a validation error, because the editor can change payout wallets. See [Dashboard › The configuration editor](dashboard.md#the-configuration-editor) and [Operations › The config-editor runner](operations.md#the-config-editor-runner). |
