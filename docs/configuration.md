...
| `tari.mem_limit` | `auto` | A **safety ceiling** on the Tari container's memory. Tari's memory grows over time (it's commonly seen using 10+ GB), and on a small host an unbounded Tari can OOM the whole stack. This cap lets Tari restart *itself* cleanly under pressure — Monero/P2Pool keep mining throughout — instead of taking the machine down. `auto` sizes it from the RAM actually free for normal use: it first subtracts the ~6 GB reserved for **RandomX HugePages** (see [HugePages](operations.md#troubleshooting)), then holds back ~25% of what's left (≥2 GB) for Monero/P2Pool/Tor/the OS/page cache, and gives Tari the rest (floored at 2 GB). With HugePages on that's roughly **7.5 GB on a 16 GB host, 19 GB on 32 GB**; with HugePages disabled (`--skip-optimize`) it's ~75% of RAM. To bound it tighter (e.g. to free RAM for other apps), set any Docker memory value such as `"8g"` or `"4096m"`; raise it again if Tari restarts too often. (Tari officially needs ≥4 GB, 8 GB+ recommended. **An 8 GB host can't fit both the HugePages pool and Tari** — use 16 GB+ or `--skip-optimize`. Set the cap too low and Tari may OOM-loop during initial sync. A clean restart relies on disabling swap, which needs `swapaccount=1` on older cgroup-v1 hosts; cgroup-v2 honors it by default.) |
