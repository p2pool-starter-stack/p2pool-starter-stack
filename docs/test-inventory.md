...
**Totals:** 371 dashboard unit tests · 12 contract tests · 20 frontend
...
| Tier | Suite | Cases |
|---|---|---|
| 1 — Unit | dashboard pytest | 371 |
| 1 — Unit | frontend (node --test) | 20 |
...
#### tests/collector/test_pools.py — 14
...
#### tests/service/test_storage_service.py — 20
- test_get_tiers
...
- test_share_points_sparse_and_lifted
- test_share_offset_floor_when_value_zero
...
### Frontend logic (node --test) — 20 tests
...
### `pithead` shell suite (tests/stack/run.sh) — 14 sections
...
- secrets preserved (token + onions)
...
### Harness self-test (tests/integration/selftest.sh) — 10 sections
