...
.PHONY: test test-dashboard test-stack test-compose test-security test-integration test-integration-selftest test-fakes test-mini-stack lint

test: lint test-dashboard test-stack test-compose test-security test-integration-selftest test-fakes ## Run everything that doesn't need a server/docker
...
test-security: ## Assert docker-compose.yml hardening invariants (regression guard for #90)
	bash tests/stack/test_security.sh
...
lint: ## shellcheck the stack scripts
	shellcheck --severity=warning pithead tests/stack/run.sh tests/stack/test_compose.sh \
		tests/stack/test_security.sh tests/inventory.sh tests/integration/*.sh \
		tests/integration/mini-stack/*.sh
