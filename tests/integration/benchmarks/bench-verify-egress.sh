...
ARM="${1:-}"; case "$ARM" in tor|clearnet) shift ;; *) echo "usage: bench-verify-egress.sh <tor|clearnet> [--dir DIR] [--prefix P] [--polls N] [--interval S] [--min-hits K]" >&2; exit 2 ;; esac
DIR="/srv/code/pithead"; PREFIX="172.28.0"; POLLS=4; INTERVAL=10; MIN_HITS=2
while [ $# -gt 0 ]; do
    case "$1" in
        --dir)      DIR="$2"; shift 2 ;;
...
[ "$POLLS" -ge 1 ] 2>/dev/null || { echo "--polls must be >= 1" >&2; exit 2; }
...
for c in $APPS; do
    ...
    if [ "$ARM" = "tor" ]; then
        if [ "$n" -eq 0 ]; then echo "  ✓ $c: no persistent public connections — all egress via Tor"
        else echo "  ✗ $c: $n PERSISTENT PUBLIC connection(s) — CLEARNET LEAK:"; printf '%s\n' "$rows" | sed 's/^/        /'; fail=1; fi
    else
        if [ "$n" -gt 0 ]; then echo "  ✓ $c: $n persistent public connection(s) — clearnet, as expected for this arm"
        else echo "  · $c: no persistent public connections (still Tor / idle — expected for monerod & tari)"; fi
    fi
done
tcid=$(cid_of tor); tn=$(public_conns "$tcid" | sed 's/:.*//' | sort -u | grep -c . || true)
...
if [ "$ARM" = "tor" ] && [ "$fail" -ne 0 ]; then
    echo "[verify-egress] FAIL — persistent clearnet leak(s) above; the 'all-Tor' arm is not clean." >&2; exit 1
fi
