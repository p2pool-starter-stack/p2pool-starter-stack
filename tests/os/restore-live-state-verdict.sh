...
# `podman ps` + dashboard `/api/state`) and tests/stack/run.sh's fixture unit test (drives it
...
# live state — p2pool's own stratum stats (mining_dashboard/collector/pools.py's
# get_stratum_stats, surfaced at /api/state's .stratum.wallet), not a re-read of the archive.

# $1 = `podman ps --format '{{.Names}}'` output (space/newline-joined; may be empty/unreachable)
# $2 = /api/state's .stratum.wallet (live; empty/"Unknown"/"null" all count as unreadable)
