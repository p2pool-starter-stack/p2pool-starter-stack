...
# flag in the address slot is kept, the rule the redactor already applies to a malformed argv.
...
assert_contains "off: a flag in the address slot is kept, not swallowed" "$(PATH="$TO_PE:$PATH" TARI_MODE=off bash "$ROOT/build/p2pool/entrypoint.sh" --merge-mine tari://172.28.0.27:18142 --stratum 0.0.0.0:3333 2>&1)" "ARG=[--stratum]"
assert_contains "local (any value but off): argv untouched" "$(PATH="$TO_PE:$PATH" TARI_MODE=local bash "$ROOT/build/p2pool/entrypoint.sh" --merge-mine tari://172.28.0.27:18142 12TariPayout 2>&1)" "ARG=[12TariPayout]"
assert_contains "unset (a 1.x .env): argv untouched" "$(PATH="$TO_PE:$PATH" env -u TARI_MODE bash "$ROOT/build/p2pool/entrypoint.sh" --merge-mine tari://172.28.0.27:18142 12TariPayout 2>&1)" "ARG=[12TariPayout]"
