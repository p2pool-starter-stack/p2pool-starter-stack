...
# #1443: the three cases above prove the MECHANISM — a marker mutation_lock_acquire exports
# survives a real process boundary. They do not prove the WIRING at the three places that
# actually build a re-invoked child: run_chain's `bash "$self" "$c"`, control_lifecycle's
# `"$self" restart` / `"$self" apply -y`, and control_backup's `"$self" backup -y`.
...
# dispatch. The control is the same one above: strip the marker and the SAME real re-invocation
# must block on its own ancestor instead of proceeding.
...
reinvoke_wiring_probe() { # <site: chain|restart|backup> <1=keep marker|0=strip> -> "<applied?>|<blocked-on-the-lock?>"
    ...
    (
        ...
        # shellcheck disable=SC1090
        source "$STACK"
        # pithead:14 is `set -Eeuo pipefail`; sourcing it turns errexit on HERE too. See the same
        # note on lock_reinvoke_probe above.
        set +e
