...
- **Dashboard "new release available" badge (#224).** When `dashboard.check_for_updates` is enabled,
  the dashboard periodically checks GitHub for the latest Pithead release and, if it's newer than the
  running version, shows a header badge next to the version badge — **"New release vX.Y.Z available ↗"**
  — linking straight to the release page. **Notify-only:** it never updates anything; you upgrade with
  `./pithead upgrade` on your own terms (the one-click upgrade is the separate #59). **Off by default**
  so the appliance never contacts GitHub unbidden; when on, the check is **routed over Tor** (the same
  bridge SOCKS as the XvB fetch, so it can't reveal your IP to GitHub), cached hourly, and fails
  silently offline. Documented in `docs/configuration.md` + `docs/privacy.md`.
