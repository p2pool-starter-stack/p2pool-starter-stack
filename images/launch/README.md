...
Marketing/launch visuals for the Pithead dashboard (Issue #80). Static views ship in **dark** and
...
**These are illustrative mockups.** Static views are rendered from the **real dashboard UI** (the
shipping Preact components + `dashboard.css`) fed a **synthetic `/api/state`** payload, so they are
...
> **`social-preview.png` is not wired up automatically** — GitHub's repo social-preview image can
