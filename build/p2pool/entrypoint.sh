...
# tari.mode "off" (#1855, #1903): the host renders TARI_MODE=off and starts no Tari node, but this
# argv still carried `--merge-mine <url> <address>`, so P2Pool merge-mined against a node that was
# not there. Drop the triple, in either spelling (`--merge-mine URL ADDR`, `--merge-mine=URL ADDR`),
# BEFORE the Tor block below reads argv, so its merge-mine bridge cannot fire on it either. Only the
# literal "off" strips: unset (a 1.x .env that predates the key) or any other value leaves argv
# exactly as it arrived. A token starting with `-` in a value position is kept, the same rule
# _redact_argv applies: a malformed argv must not swallow the next flag NAME.
