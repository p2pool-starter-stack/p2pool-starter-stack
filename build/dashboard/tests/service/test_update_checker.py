...
from mining_dashboard.service.update_checker import (
    parse_semver, compute_update, GitHubReleaseClient, UpdateChecker,
)
...
class TestComputeUpdate:
    def test_newer_returns_payload(self):
        ...
        assert out == {"available": True, "latest": "v0.2.0", "url": "https://x/releases/tag/v0.2.0"}
...
class TestGitHubReleaseClient:
    ...
    def test_parses_tag_and_url(self):
        ...
        with patch("mining_dashboard.service.update_checker.requests.get",
                   return_value=self._resp(200, {"tag_name": "v1.4.0", "html_url": "https://h/v1.4.0"})) as g:
            assert c.latest_release() == {"tag": "v1.4.0", "url": "https://h/v1.4.0"}
            # routed through the Tor proxy
            assert g.call_args.kwargs["proxies"] == {"http": "socks5h://t:9050", "https": "socks5h://t:9050"}

    def test_non_200_is_silent_none(self):
        c = GitHubReleaseClient("u")
        with patch("mining_dashboard.service.update_checker.requests.get", return_value=self._resp(404)):
    ...
    def test_missing_fields_is_none(self):
        c = GitHubReleaseClient("u")
        with patch("mining_dashboard.service.update_checker.requests.get",
                   return_value=self._resp(200, {"tag_name": "v1.0.0"})):   # no html_url
            assert c.latest_release() is None
