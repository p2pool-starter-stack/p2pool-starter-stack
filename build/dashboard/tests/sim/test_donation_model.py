...
FIELD = dict(target_hr=10_000, current_hr=46_300, measurement="fixed",
             p2pool_difficulty=DIFFICULTY, cycles=3 * CYCLES_PER_DAY)
...
class TestHoldsTierMinimumWaste:
    ...
    def test_more_headroom_means_more_p2pool(self, current_hr, expected_p2pool):
        sc = Scenario(name="hr", target_hr=10_000, current_hr=current_hr,
                      measurement="fixed", p2pool_difficulty=DIFFICULTY,
...
class TestSelfCalibration:
    ...
    def test_holds_tier_across_credit_factor(self, k):
        sc = Scenario(name=f"k{k}", target_hr=10_000, current_hr=46_300,
                      measurement="fixed", credit_factor=k, p2pool_difficulty=DIFFICULTY,
    ...
    def test_overcredit_frees_p2pool(self):
        ...
        lo = run_algo(Scenario(name="k1", target_hr=10_000, current_hr=46_300,
                               measurement="fixed", credit_factor=1.0,
                               p2pool_difficulty=DIFFICULTY, cycles=4 * CYCLES_PER_DAY))
        hi = run_algo(Scenario(name="k3", target_hr=10_000, current_hr=46_300,
                               measurement="fixed", credit_factor=3.0,
                               p2pool_difficulty=DIFFICULTY, cycles=4 * CYCLES_PER_DAY))
    ...
    def test_stable_under_lag(self, semantics):
        ...
        sc = Scenario(name="lag", target_hr=10_000, current_hr=46_300,
                      measurement=semantics, report_lag_cycles=6, credit_factor=2.0,
                      p2pool_difficulty=DIFFICULTY, cycles=5 * CYCLES_PER_DAY)
        r = run_algo(sc)
        assert r.tier_held()
...
class TestVipReserve:
    ...
    def test_low_tier_high_difficulty_caps_donation_for_vip(self):
        ...
        sc = Scenario(name="tightvip", target_hr=10_000, current_hr=46_300,
                      measurement="fixed", p2pool_difficulty=300_000_000,
                      cycles=3 * CYCLES_PER_DAY)
        r = run_algo(sc)
        assert r.vip_held()
...
class TestDisturbanceRecovery:
    def test_recovers_after_worker_drop(self):
        ...
        sc = Scenario(name="drop", target_hr=10_000, current_hr=46_300, warm_avg=10_300,
                      measurement="fixed", p2pool_difficulty=DIFFICULTY,
                      cycles=4 * CYCLES_PER_DAY,
