...
# 14, not 16: the docs' "16 GB or more" machine REPORTS ~15.x usable (reserved memory,
# GiB-vs-GB), so a threshold equal to the spec put a permanent Low-RAM badge on every
# spec-compliant box. 14 flags genuinely under-provisioned hardware (8/12 GB) only.
LOW_RAM_GB = int(float(os.environ.get("LOW_RAM_GB", 14)))
